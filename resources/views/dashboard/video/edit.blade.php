...
<div class="max-w-4xl">
    ...
    <form action="{{ route('dashboard.videos.update', $video) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-md p-8">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <textarea id="description" name="description" rows="4" 
                      ...
                      placeholder="Brief description of the video...">{{ old('description', $video->description) }}</textarea>
        ...
        <div class="mb-6">
            ...
            <select id="category" name="category" 
                ...
                <option value="Sermons" {{ old('category', $video->category) == 'Sermons' ? 'selected' : '' }}>Sermons</option>
                <option value="Testimonies" {{ old('category', $video->category) == 'Testimonies' ? 'selected' : '' }}>Testimonies</option>
                <option value="Special Events" {{ old('category', $video->category) == 'Special Events' ? 'selected' : '' }}>Special Events</option>
                <option value="Cultural Events" {{ old('category', $video->category) == 'Cultural Events' ? 'selected' : '' }}>Cultural Events</option>
                <option value="Teachings" {{ old('category', $video->category) == 'Teachings' ? 'selected' : '' }}>Teachings</option>
                <option value="Worship" {{ old('category', $video->category) == 'Worship' ? 'selected' : '' }}>Worship</option>
                <option value="Youth Ministry" {{ old('category', $video->category) == 'Youth Ministry' ? 'selected' : '' }}>Youth Ministry</option>
                <option value="Health & Safety" {{ old('category', $video->category) == 'Health & Safety' ? 'selected' : '' }}>Health & Safety</option>
        ...
        <div class="mb-6">
            ...
            <div class="flex gap-4">
                ...
                <label class="flex items-center">
                    <input type="radio" name="video_type" value="vimeo" {{ old('video_type', $video->video_type) == 'vimeo' ? 'checked' : '' }} 
                           class="mr-2" onchange="toggleVideoFields()">
        ...
        <div id="video_url_field" class="mb-6" style="display: {{ in_array(old('video_type', $video->video_type), ['youtube', 'vimeo']) ? 'block' : 'none' }}">
        ...
        <!-- Video File Upload -->
        <div id="video_file_field" class="mb-6" style="display: {{ old('video_type', $video->video_type) == 'upload' ? 'block' : 'none' }}">
            <label for="video_file" class="block text-sm font-semibold text-gray-700 mb-2">
                <i class="fas fa-file-video text-indigo-600 mr-2"></i> Video File (Optional - Replace existing)
            </label>
            @if($video->video_file)
                <p class="text-sm text-gray-600 mb-2">Current: {{ basename($video->video_file) }}</p>
            @endif
        ...
        <div class="mb-6">
            <label class="block text-sm font-semibold text-gray-700 mb-4">
                <i class="fas fa-image text-indigo-600 mr-2"></i> Current Thumbnail
            </label>
            <div class="mb-4">
                <img src="{{ $video->thumbnail_url }}" alt="{{ $video->title }}" class="max-h-48 rounded-lg shadow-md">
            </div>
        ...
        <div class="mb-6">
            ...
            <input type="date" id="publish_date" name="publish_date" value="{{ old('publish_date', $video->publish_date ? $video->publish_date->format('Y-m-d') : '') }}" 
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
        ...
        <div class="mb-6 space-y-3">
            <label class="flex items-center">
                <input type="checkbox" name="featured" value="1" {{ old('featured', $video->featured) ? 'checked' : '' }} class="h-4 w-4 text-indigo-600 rounded">
            ...
            <label class="flex items-center">
                <input type="checkbox" name="published" value="1" {{ old('published', $video->published) ? 'checked' : '' }} class="h-4 w-4 text-indigo-600 rounded">
                <span class="ml-2 text-sm text-gray-700">
