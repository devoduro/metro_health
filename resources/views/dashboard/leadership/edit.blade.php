...
@section('content')
<div class="max-w-4xl">
    <form action="{{ route('dashboard.leadership.update', $leadership) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-md p-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $leadership->name) }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent @error('name') border-red-500 @enderror">
            ...
            <div>
                ...
                <select id="title_id" name="title_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent @error('title_id') border-red-500 @enderror">
                    ...
                    @foreach($titles as $title)
                        <option value="{{ $title->id }}" {{ old('title_id', $leadership->title_id) == $title->id ? 'selected' : '' }}>
                            {{ $title->title }}
        ...
        <div class="grid grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <input type="email" id="email" name="email" value="{{ old('email', $leadership->email) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent @error('email') border-red-500 @enderror">
                @error('email') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="phone" class="block text-sm font-semibold text-gray-700 mb-2">Phone</label>
                <input type="tel" id="phone" name="phone" value="{{ old('phone', $leadership->phone) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent @error('phone') border-red-500 @enderror">
        ...
        <div class="mb-6">
            ...
            <textarea id="bio" name="bio" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent @error('bio') border-red-500 @enderror">{{ old('bio', $leadership->bio) }}</textarea>
        ...
        <div class="grid grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <select id="year" name="year" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent @error('year') border-red-500 @enderror">
                    ...
                    @foreach($years as $year)
                        <option value="{{ $year->year }}" {{ old('year', $leadership->year) == $year->year ? 'selected' : '' }}>
                            {{ $year->yearRange }}
                            @if($year->description)
                                ({{ $year->description }})
                            @endif
            ...
            <div>
                ...
                @if($leadership->image)
                    <div class="mt-3">
                        ...
                        <img src="{{ asset('storage/' . $leadership->image) }}" alt="{{ $leadership->name }}" class="h-24 w-24 object-cover rounded-lg border">
                    </div>
                @endif
            ...
            <div>
                ...
                <input type="number" id="display_order" name="display_order" value="{{ old('display_order', $leadership->display_order) }}" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent @error('display_order') border-red-500 @enderror">
        ...
        <div class="mb-6 space-y-3">
            <label class="flex items-center">
                <input type="checkbox" name="active" value="1" {{ old('active', $leadership->active) ? 'checked' : '' }} class="h-4 w-4 text-cyan-600">
                <span class="ml-2 text-sm text-gray-700">Active (Display on website)</span>
            </label>
            
            <label class="flex items-center">
                <input type="checkbox" name="is_current" value="1" {{ old('is_current', $leadership->is_current) ? 'checked' : '' }} class="h-4 w-4 text-cyan-600">
